...
DROP FUNCTION IF EXISTS public.handle_loyalty_redemption(p_user_id uuid, p_points_to_redeem integer, p_order_id uuid, p_gift_id uuid);

-- Ensure the text version is correctly defined
CREATE OR REPLACE FUNCTION public.handle_loyalty_redemption(p_user_id uuid, p_points_to_redeem integer, p_order_id uuid, p_gift_id text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
