-- Security fixes for TVANAMM Tea project
...
-- Fix 2: Strengthen RLS policies for financial tables

-- Drop overly permissive policies and create more restrictive ones for loyalty_points
...
CREATE POLICY "Authenticated system can update loyalty points" ON public.loyalty_points
FOR UPDATE 
USING (auth.uid() IS NOT NULL);

CREATE POLICY "Authenticated system can insert loyalty points" ON public.loyalty_points
FOR INSERT 
WITH CHECK (auth.uid() IS NOT NULL);

-- Drop overly permissive policies and create more restrictive ones for loyalty_transactions
DROP POLICY IF EXISTS "System can manage loyalty transactions" ON public.loyalty_transactions;
...
CREATE POLICY "Authenticated system can insert loyalty transactions" ON public.loyalty_transactions
FOR INSERT 
WITH CHECK (auth.uid() IS NOT NULL);

-- Drop overly permissive policies and create more restrictive ones for payment_transactions
DROP POLICY IF EXISTS "System can manage payment transactions" ON public.payment_transactions;
...
CREATE POLICY "Authenticated system can insert payment transactions" ON public.payment_transactions
FOR INSERT 
WITH CHECK (auth.uid() IS NOT NULL);

CREATE POLICY "Authenticated system can update payment transactions" ON public.payment_transactions
FOR UPDATE 
USING (auth.uid() IS NOT NULL);