-- Fix loyalty points redemption function
...
DECLARE
  ...
  result JSONB;
BEGIN
  -- Get user's current points and TVANAMM ID (check both franchises and profiles tables)
  SELECT lp.current_balance, COALESCE(f.tvanamm_id, p.tvanamm_id) 
  INTO current_points, franchise_tvanamm_id
  FROM public.loyalty_points lp
  LEFT JOIN public.franchises f ON f.user_id = lp.user_id
